...
Pixel_features=pd.read_csv('Pixel_9by12_47to98_concatenated.csv')
#Pixel_features=pd.read_csv('Pixel_12by16_47to98_concatenated.csv')
#Pixel_features=pd.read_csv('Pixel_6by8_47to98_concatenated.csv')
#Pixel_features=pd.read_csv('Pixel_3by4_47to98_concatenated.csv')
...
normalized_first_den = Pixel_features.iloc[:, :int(Pixel_features.shape[1]/4)] / den_cap 
normalized_first_vol = Pixel_features.iloc[:, int(Pixel_features.shape[1]/4):2*int(Pixel_features.shape[1]/4)] / vol_cap
normalized_second_den = Pixel_features.iloc[:, 2*int(Pixel_features.shape[1]/4):3*int(Pixel_features.shape[1]/4)] / den_cap
normalized_second_vol = Pixel_features.iloc[:, 3*int(Pixel_features.shape[1]/4):] / vol_cap

Normalized_features = pd.concat([normalized_first_den, normalized_second_den, normalized_first_vol,normalized_second_vol], axis=1)

Normalized_features['nor_dayofweek'] = new_datetime['normalized_dayofweek'] 
Normalized_features['nor_hour'] = new_datetime['normalized_hour']
#-----------------------------------
#-----------------------------------
...
def class_accuracy(class_id):
    def accuracy(y_true, y_pred):
        ...
        accuracy = correct_predictions / (total_class_count + tf.keras.backend.epsilon())
        return accuracy
    accuracy.__name__ = f'accuracy_class_{class_id}'
    return accuracy
...
with strategy.scope():
    ...
    accuracy_metrics = [class_accuracy(i) for i in range(num_classes)]

    model.compile(
        optimizer="adam",
        loss="categorical_crossentropy",
        metrics=[*precision_metrics, *recall_metrics, *accuracy_metrics]
    )
...
dfRNN_kernel.to_csv('9by12_RNN_kernel_t3.csv', index=False) 
dfRNN_rekernel.to_csv('9by12_RNN_rekernel_t3.csv', index=False) 
...
df_pred.to_csv('pred_9by12_multigpus_4CLASSES_10AM_t3.csv', index=False) 
...
df_divided_array.to_csv('PEPE_9by12_multigpus_4CLASSES_10AM_t3.csv', index=False) 
